...
const Quiz = ({ level, deckid }: any) => {
  let kanjis = KanjiDictionary.getKanjiByLevel(level).slice(
    deckid * 20,
    deckid * 20 + 20,
  );
  const [
      ...
      <div className="flex flex-col gap-3 mt-32">
        <div className="flex gap-3 justify-center flex-wrap">
          {currentQuestion.readings_on.map((reading: any, index: number) => (
            <span key={index} className="bg-sky-800 py-1 px-3 rounded">
              {reading}
            </span>
          ))}
          {currentQuestion.readings_kun.map((reading: any, index: number) => (
            <span key={index} className="bg-sky-600 py-1 px-3 rounded">
              {reading}
            </span>
          ))}
        </div>
